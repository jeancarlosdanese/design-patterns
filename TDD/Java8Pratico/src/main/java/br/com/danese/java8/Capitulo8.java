...
class Capitulo8 {
	public static void main(String... args) throws Exception {
		...
		usuarios.stream()
				.filter(u -> u.getPontos() > 100)
				.peek(System.out::println); // precisa
											// chamar
											// terminal!
...
//		strings.forEach(System.out::println);
		...
		/*List<String> linhas = Files.list(Paths.get(path)).filter(p -> p.toString().endsWith("12.java"))
				.flatMap(p -> lines(p))
				.collect(Collectors.toList());*/
		
		Stream<String> linhas = 
				Files.lines(Paths.get(path + "/Capitulo12.java"))
				.filter(l -> !l.isEmpty());
		
		linhas.forEach(System.out::println);
		
		IntStream chars = Files.list(Paths.get(path))
				.filter(p -> p.toString().endsWith(".java"))
				.flatMap(p -> lines(p))
