...
public class Capitulo11 {
	...
	public static void main(String... args) throws Exception {
		...
		BigDecimal p1total = payment1.getProducts().stream().map(Product::getPrice).reduce(BigDecimal.ZERO,
		...
		Map<Product, BigDecimal> totalValuePerProduct2 = payments.stream().flatMap(p -> p.getProducts().stream())
				.collect(Collectors.toMap(Function.identity(), Product::getPrice, BigDecimal::add));

		totalValuePerProduct2.entrySet().stream().sorted(Comparator.comparing(Map.Entry::getValue))
				.forEach(System.out::println);
		...
		customerToProducts.entrySet().stream().sorted(Comparator.comparing(e -> e.getKey().getName()))
				.forEach(System.out::println);
		...
		System.out.println(ChronoUnit.MONTHS.between(s1.getBegin(), LocalDateTime.now()));

		System.out.println(ChronoUnit.MONTHS.between(s1.getBegin(), s1.getEnd().orElse(LocalDateTime.now())));

		// dada uma unica subscription, calcular quanto ele pagou ate hoje
		System.out.println(s1.getMonthlyFee().multiply(
				new BigDecimal(ChronoUnit.MONTHS.between(s1.getBegin(), s1.getEnd().orElse(LocalDateTime.now())))));

		// dada uma colecao de subscription, calcular quanto todos pagaram ate
		// hoje
		...
		// achar o usuario que mais pagou total de valor de subscriptions ate
		// hoje

		// achar o usuario que ficou mais meses pagando (independente dela estar
		// ativa ou nao)

		// quero map<Customer, Type>, qual é o tipo de produto que ele mais
		// comprou

		// que tipo de produto é o mais vendido (sum(VIDEO), sum(AUDIO))

		// Dado um %%Payment%%, gere a lista de %%Paths%% dos arquivos digitais
		// comprados.
	}
}
